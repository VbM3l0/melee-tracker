...
with st.form("skill_form"):
    ...
    set_custom_level = st.checkbox("I want to set a custom target level")
    
    if set_custom_level:
        desired_level = st.number_input("Target Skill Level", min_value=skill + 1, max_value=200, value=110)
    else:
        desired_level = skill + 1
...
if submitted:
    ...
    Tp_target = A * (b ** (desired_level - c) - 1) / (b - 1)
    ...
    st.write(f"**Target Skill Level:** {desired_level}")
